...
if (isset($_GET["vaccinated"])) {
    ...
    $sql = "SELECT * FROM pets WHERE vaccinated = '$vaccinated'";
    ...
    if (mysqli_num_rows($result) == 0) {
        ...
        while ($row = mysqli_fetch_assoc($result)) {
            ...
            <div class = 'col-lg-4 col-md-6 col-sm-12 p-3'>
                <div class='card' style='width: 18rem;'>
                    <img src='picture/" . $row['picture'] . "' class='card-img-top' alt='...' style='height: 250px;'>
                        <div class='card-body shadow-lg'>
                        <h5 class='card-title'>" . $row['name'] . "</h5>
                        <p class='card-text'><span class = 'fw-bold'>species : </span>" . $row['species'] . "</p>
                        <p class='card-text'><span class = 'fw-bold'>Location : </span>" . $row['location'] . "</p>
...
<?php include('navbar.php'); ?>
        <div class="hero p-4 mb-3">
            <div class ="row row-cols-4">
                ...
                <p> your are looking for Vaccinated or Not</p>
                        </strong> </h2>
                </div>
            </div>
        </div>
